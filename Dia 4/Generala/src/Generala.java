...
public class Generala {
    ...
    public String jugada(String jugada) {
        if (jugada.length() != 5) return "INVALIDO";

        int[] contador = new int[6];

        for (int i = 0; i < jugada.length(); i++) {
            char c = jugada.charAt(i);
            if (c < '1' || c > '6') return "INVALIDO";
        ...
        for (int i = 0; i < 6; i++) {
            if (contador[i] == 5) return "GENERALA";
        }

        for (int i = 0; i < 6; i++) {
            if (contador[i] == 4) return "POKER";
        }
        ...
        for (int i = 0; i < 6; i++) {
            ...
            else if (contador[i] == 2) hayDos = true;
        }
        if (hayTres && hayDos) return "FULL";
