type Item = { id: number; name: string; children?: Item[] };
...
function renderTree(items: Item[], container: HTMLElement) {
  container.innerHTML = '';
  const ul = document.createElement('ul');
  ul.className = 'tree';
  container.appendChild(ul);
  for (const item of items) ul.appendChild(renderItem(item, false));
}

function renderItem(item: Item, isChild: boolean): HTMLLIElement {
  ...
  if (selectedId === item.id) rect.classList.add('selected');

  let chevron: HTMLDivElement | null = null;
  ...
  if (!isChild && hasChildren) {
    row.addEventListener('click', () => {
      ...
      chevron?.classList.toggle('collapsed', !expanded.has(item.id));
      if (childrenWrap) {
        childrenWrap.innerHTML = '';
        if (expanded.has(item.id)) {
          const u = document.createElement('ul');
          u.className = 'tree children';
          for (const ch of item.children!) u.appendChild(renderItem(ch, true));
...
/* Acordeon */
function setupAccordion(rootSelector: string) {
  const root = document.querySelector(rootSelector);
  if (!root) return;
  root.addEventListener('click', (ev) => {
    const header = (ev.target as HTMLElement).closest('.acc-header') as HTMLElement | null;
    if (!header) return;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!treeRoot) throw new Error('No se encontró #tree-root');
